/*
 * Wrapper for CBLAS routines.
 *
 * copyright@ Yimin Zhong. <yzhong@utexas.edu>. All Rights Reserved.
 *
 */
...
void t_dgemm(scalar_t alpha, Matrix &A, Matrix &B, scalar_t beta, Matrix &C) {
    ...
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                C.row(), C.col(), A.row(), alpha, A.data(), A.row(), B.data(), B.row(), beta, C.data(), C.row());
}
